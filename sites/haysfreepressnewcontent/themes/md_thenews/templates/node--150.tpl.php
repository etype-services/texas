...
$savepayment="insert into `order`(username,email,rscode,tranId,swchKey,dtTm,amount,stan,mapCaid,status,comment) value('".$_SESSION['username']."','".$_SESSION['email']."','$rscode','$tranId','$swchKey','$dtTm','$amt','$stan','$mapCaid','$paymentStatus','$paymentComment')";
                        ...
                        /* send response */
                        $paramr=array('PublicationID' =>"$publicationid",'UserName' =>"$UserName",'SubscriptionType' =>"$planid",'rspCode' =>"$rscode",'aci'=>"$aci",'swchKey'=>"$swchKey",'tranNr'=>"$trannr",'dtTm'=>"$dtTm",'amt'=>"$pamt",'stan'=>"$stan",'auth'=>"NULL",'cardType'=>"$cardtype",'mapCaid'=>"$mapCaid",'IsSuccess'=>"$flag2",'PaymentStatus'=>"$paymentStatus",'PaymentComment'=>"$paymentComment",'SubscriptionText'=>"Subscribe",'RequestID'=>"$reqid");
